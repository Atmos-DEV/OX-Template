...
RegisterCommand('opencoffre', function()
	...
	if #(playerCoords - position) < 6 and NetworkGetEntityIsNetworked(vehicle) then
		...
		if locked == 0 or locked == 1 then
			...
			local closeToVehicle = distance < 4 and (open == 5 and (checkVehicle == nil and true or 2) or open == 4)
			if closeToVehicle then
				...
				if distance > 4 then TriggerEvent('ox_inventory:closeInventory') end
			end
